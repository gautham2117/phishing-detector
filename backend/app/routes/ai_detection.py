...
ai_detection_bp = Blueprint(
    "ai_detection_bp", __name__, url_prefix="/ai-detection"
)
...
@ai_detection_bp.route("/scan/text", methods=["POST"])
def scan_text():
...
@ai_detection_bp.route("/scan/url", methods=["POST"])
def scan_url():
    payload = request.get_json(silent=True) or {}
...
@ai_detection_bp.route("/scan/file", methods=["POST"])
def scan_file():
...
@ai_detection_bp.route("/detail/<int:scan_id>")
def detail(scan_id):
    try:
